...
export const UpdateManagerModal: React.FC<UpdateManagerModalProps> = ({ onClose }) => {
  ...
  // Force update double-click state
  const [forceUpdateReady, setForceUpdateReady] = useState<boolean>(false);
  ...
  // Force update with double-click activation
  const handleForceUpdate = useCallback(async () => {
    // First click: activate ready state
    if (!forceUpdateReady) {
      setForceUpdateReady(true);
      
      // Set timeout to reset ready state after 5 seconds
      if (forceUpdateTimeoutRef.current) {
        clearTimeout(forceUpdateTimeoutRef.current);
      }
      forceUpdateTimeoutRef.current = setTimeout(() => {
        setForceUpdateReady(false);
        forceUpdateTimeoutRef.current = null;
      }, 5000);
    ...
    // Second click: execute force update
    ...
    try {
      logApiActivity('Force Update - Request', endpoint, 'POST', requestData);
      
      const response = await api.post<UpdateApplyResponse>(endpoint, requestData);
      
      logApiActivity('Force Update - Response', endpoint, 'POST', requestData, response);
      ...
      if (response.status === 'success') {
        ...
        // Generate detailed success message with module information
        let successMessage = `✅ Force update completed successfully!\nDuration: ${finalDuration}`;
        ...
        setUpdateOutput(prev => [...prev, '', '='.repeat(50), successMessage, '='.repeat(50)]);
        toast.success('Force update completed successfully!');
        ...
        logger.error('Force update failed:', response.error);
        const startTimeToUse = updateStartTimeRef.current || updateStartTime;
        const finalDuration = startTimeToUse ? calculateDuration(startTimeToUse) : updateDuration || '0:00';
        setUpdateOutput(prev => [...prev, '', '='.repeat(50), `❌ Force update failed!\nError: ${response.error || 'Update failed'}\nDuration: ${finalDuration}`, '='.repeat(50)]);
        toast.error(response.error || 'Failed to apply force update');
      }
    } catch (error) {
      logApiActivity('Force Update - Error', endpoint, 'POST', requestData, undefined, error);
      logger.error('Error applying force update:', error);
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const startTimeToUse = updateStartTimeRef.current || updateStartTime;
      const finalDuration = startTimeToUse ? calculateDuration(startTimeToUse) : updateDuration || '0:00';
      setUpdateOutput(prev => [...prev, '', '='.repeat(50), `❌ Force update failed!\nError: ${errorMessage}\nDuration: ${finalDuration}`, '='.repeat(50)]);
      toast.error('Failed to apply force update');
    } finally {
      setIsApplying(false);
      updateStartTimeRef.current = null;
    }
  }, [api, toast, forceUpdateReady, handleCheckUpdates, loadModules, loadSystemInfo, calculateDuration, logApiActivity]);
